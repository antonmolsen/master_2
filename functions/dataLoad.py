...
# - DATA LOAD FUNCTION - DATA LOAD FUNCTION - DATA LOAD FUNCTION - DATA LOAD FUNCTION
def dataLoad(filename):
   ...
   # we read in our data
    temp_file = pd.read_csv(filename,header=None, delim_whitespace=True, dtype=float) # delimiter is whitespace
    
    arr = np.array(temp_file) # numpy array, every row is true
    # made to nump array because i do not understand DataFrames 
    
    for i in range(len(arr[:,1])): #length of dataset
        if len(np.array(arr)[i,:]) == 3: # only use rows with 3 elements
            
            if 10>= np.array(arr)[i,0] or np.array(arr)[i,0]>=60 : #temperature boundary in 0'th collumn
                read = False
